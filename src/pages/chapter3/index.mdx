...
Introducing the next chapter at the beginning of the chapter that you're writing has to be the worst way to write an introduction to a chapter. But I just CAN'T WAIT to tell you that in the coming chapter we will learn how to draw using Math and JavaScript.
...
The cool thing about exterior angles is that if club them together they'd form a whole 3 hundred and 60 degrees.
...
Let's start breaking it down. First is the base case, which we will get to in a bit. Beyond that, the retrieval of `firstLetter` and `lastLetter` is straightforward, we're just picking characters at the first (`[0]`) and last (`[string.length - 1]`) indices of the string that is passed as the argument.
...
[ART illustrating what remaining string is]

And to do that we're using JavaScript's [`String.prototype.slice`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/slice) method.
...
A special thing about `.slice` is that it also accepts negative numbers for parameters. When it notices a negative number, it adds that number to the length of the original string, to keep everything within the bound of indices.
...
In the next two chapters, we'll delve into something a little different from what we've been doing so far. In the two chapters that follow you'll be learning Maths and HTML5 canvas and you'll draw recursive art in your browser window.
...
How many cannonballs will a pyramid of height 7 contain? and of height 10? Your recursive function will calculate the number of cannonballs in the pyramid of *any* given height. 
...
This chapter was all about revisiting and polishing the heuristics that we have used to write all our recursive functions. Once you have the heuristics and the basic understanding taken care of, we can move on to more advanced concepts, which I probably don't need to mention again because my eager self has already that leaked that information multiple times in this same chapter. 

It's time to move on to Graphical Recursion.

<Callout>
Want to read the next chapter early, or help me review and write it? I have made a discord server where I post progress on the next chapter and also share PDFs for early review. If that's something you're interested in, come hang!
</Callout>