...
All that we'd have to do is switch the position of the letters at the first and the last positions. `L` and `P` in the case of the word LAPTOP (I'll handle the rest)
...
But, we're using JavaScript's [`String.prototype.slice`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/slice) method for the task. Passing around negative arguments in `.slice()` can possibly throw you off a little if you don't understand it well, so here's a video of me expalining it in case you ever needed it. 

Notice we also have the method `getRemainingString`, the task of `getRemainingString` is to return the string that remains after you remove the first and last letters/characters from it. And to do this we'll again, take help of the `.slice` method, like so.
...
By now you should be in the early stages of developing the recursive lens and identifying the 2 crucial parts of a recursive solution. The base case and the trust in the solution a.k.a "Give me this, and I'll handle the rest".
...
![ART ISOMETRIC LAYERS OF CANNON ALLS](13.png)
