...
Now one thing we have already made abundantly clear is that the Sum of all exterior angles is 360 degrees.
...
So, for any polygon (we're getting there)
...
And now you know the formula in which you can simply plug in the number of sides and be handed the answer came from. When you understand why a formula works, it sticks better and for longer.
...
And just like that, BE DONE!

The Give me this and I'll handle the rest rule works because like any other recursive problem, the problem of reversing a word is decomposable. In the sense that it can be broken down into smaller magnitudes and still be solved by the exact same steps. And the end each result from can be combined into the final result.

Let's start coding the recursive solution. We'll call our function `reverseString`. Why not `reverseWord`? Because words are called strings in programming lingo and more or less they are the same thing.

So, `reverseString`. Alright?

`reverseString` takes in a string/word as an argument, recursively reverses it, and returns the reversed version of the string.

Here it is, and it's beautiful.

```javascript
function reverseString(string) {
  // Get the extreme letters
  let firstLetter = string[0];
  let lastLetter = string.slice(-1);
  let remainingString = getRemainingString(string); // remaining string after getting rid of first and last letters
    
  return lastLetter + reverseString(remainingString) + fristLetter;
}
```

Now let's start breaking it down. The retrieval of firstLetter is fairly straightforward. The lastLetter can be also got to by `string[string.length - 1]`

[ART]

But, we're using JavaScript's `String.prototype.slice` method for the task. We'll take a short detour to the `.slice` method to understand how it works and then quickly jump back to breaking down the solution. Before we do that though, notice we also have the method `getRemainingString`, the task of `getRemainingString` is to return the string that remains after you remove the first and last letters/characters from it. And to do this we'll again, take help of the `.slice` method, like so.
...
If nothing's making sense yet, no worries, it all will in a while. Ready for the `.slice` crash course? Here we go!

[think if you really need to put it in here or not]

Now that we know how `.slice() `works, we can move to the next line, which essentially is the "..I'll handle the rest". What we're doing here is taking the first and last letters and switching their positions.

```javascript
return lastLetter + reverseString(remainingString) + fristLetter;
```
Notice how `lastLetter` is at the beginning of the sentence and `firstLetter` is at the end. Yes, in JavaScript you can use + to add strings together.
Also, notice that the recursive code we just wrote has a missing base case. To figure what the base case should be, always start by noticing what's changing in each call. With `reverseString`, it's the parameter string of course. With every call, we're removing two letters and hence the length of the string parameter is decreasing and will eventually become 0 or 1. That is exactly where our base case is.
...
function reverseString(string) {
  ...
  let firstLetter = string[0];
  let lastLetter = string.slice(-1);
  let rest = getRestString(string)// Rest of the string

  return lastLetter + reverseString(rest) + firstLetter;
}
```
...
By now you should be in the early stages of developing the recursive lens and identifying the crucial 3 parts of a recursive solution. The base case, the trust in the solution a.k.a "Give me this, and I'll handle the rest".

At this stage, having a little itch to solve another problem recursively is normal. I know you must be feeling full with ammo and arsenal ready to tackle your next recursive problem, so I am going to give you one. The problem is of counting cannonballs. I'll define the problem here in a bit, and we'll get back to solving it together after the next two chapters. In the next two chapters, we'll delve into something a little different from what we've been doing so far. In the two chapters that follow you'll be learning Maths and HTML5 canvas and you'll draw recursive art in your browser window.
Until then, I'll leave you with the cannonballs problem.
What you have is a number of spherical cannonballs stacked to form a pyramid with a square base. Your job is to write a recursive function that takes in the height of the pyramid as the only argument and returns the number of cannonballs in that pyramid.
...
[ART]
The program you will write will recursively calculate the number of cannonballs in the pyramid of a given height. By height what I simply mean is number of layers of cannonballs stacked in that pyramid. In the above example the pyramid has 3 layers, hence it's of height 3.
And the pyramid of height 3, if you count, has 14 cannonballs.
[ART]
How many cannonballs will a pyramid of height 7 contain? and of height 10? Your recursive function will calculate the number of cannonballs in the pyramid of any given height. Here's a hint, since the pyramid is of a square base, it's easy to figure out the number of cannonballs in any given layer.
Now onto new adventures with math and drawing and drawing and math … whatever…
