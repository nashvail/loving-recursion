...
Let's start breaking it down. The retrieval of `firstLetter` is fairly straightforward. The `lastLetter` can be also got to by `string[string.length - 1]` but we're using JavaScript's [`String.prototype.slice`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/slice) method for the task. Passing around negative arguments in `.slice()` can possibly throw you off a little if you don't understand it well, so here's a video of me explaining it.
...
We are again passing in negative arguments to `.slice` for the job, if that's confusing remember the video or just read the docs.
