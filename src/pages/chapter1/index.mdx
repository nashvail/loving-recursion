...
Now, we'll take a moment here to get acquainted with an undeniably powerful mother-fucker, the base-case. Search for a recursive solution often starts with a search for the base-case.

Now keep in mind I cannot conjure up a potion in one paragraph and have it absorbed into your skulls and explain the concept of base-case once and for all. If I could, I would, trust me, but base-case is one of those things you have to repeatedly observe in its natural habitat to get a proper hang of.
...
This means `isOdd` will return whatever `isEven` returns but will do a NOT operation on the result first. That is, if `isEven` returns true, `!` will make it a `false` and vice versa. We're jumping a little ahead of ourselves here since we have not yet reached the part of the diagram where the answer begins to return, but this is an important detail to keep in mind nonetheless.

![](11.png)

From inside `isOdd(1)` `isEven` is executed with an unchanged parameter, `1`.
Inside `isEven` it's the same story, one check to see if the parameter is `0` if not, reduce `1`, handle control to `isOdd`.

[art]

Inside, `isOdd(0)`, `isEven` is executed with `0`, which if you remember is the base case. `isEven` performs a check to see if the parameter is 0, if it is returns `true` (since 0 is even). This answer then bubbles up the execution stack, effectively replacing the expression beside `return` in each function call with a value, allowing each function to return something to the parent function that called it.

Eventually, the original function `isEven(2)` gets a value to return, and if everything was coded properly with correct logical insights, a correct answer is supplied.
...
The checks are easy to put in and adding such naive complexity in the explanation will only cloud the meat that I want to actually share.

And before I go, hint hint, wink wink, if you want to cleanse for negative numbers check out the <code class="language-text"><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/abs">Math.abs</a></code> function. And remember, sign of a number does not effect its parity(the quality of being even or odd).
