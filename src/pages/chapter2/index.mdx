...
Once we have filled in the `moveStack` function with the correct algorithm between the braces, executing ... 
...
To begin with the `n` discs are stacked on a spire we have name 'A', we could've name it Ahmed, or Anita but A is a good enough name for a spire. Our goal is to move the stack of discs from Spire A (`fromSpire`) to Spire B (`toSpire`) while using Spire C as the temporary spire (`tempSpire`).

[ART GENERAL SETUP]

With this setup, the steps involved in the general solution are:

<b><u>Step-1:</u></b> Take the tower of `n-1` discs from `fromSpire` and move it to *tempSpire*. (This will leave a single disc on `fromSpire`).
...
Before we proceed, here's a working version of the whole program. Peep the console.
