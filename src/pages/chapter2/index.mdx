...
Look at the 2 discs problem that we solved earlier. In that problem, leaving just one disc behind, we took the rest of the stack (yes, just 1 disc tall) and transferred it to Spire C. Then, we moved the remaining one disc on Spire A to Spire B, and in the final step moved the rest of the stack- which was just one disc tall - to Spire B and finally had the entire tower on Spire B. All of that without breaking any of the two rules.
...
Then, we'll move the tower with `n-1` discs on `tempSpire` (Temp Sire) to `toSpire`
